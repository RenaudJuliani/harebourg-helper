...
describe('entitySlice', () => {
  ...
  it('placeEntity on floor succeeds', () => {
    const s = makeStore();
    s.getState().placeEntity('harebourg', { x: 8, y: 5 });
  ...
  it('placeEntity on obstacle is rejected silently', () => {
    ...
    s.getState().placeEntity('ally', { x: 8, y: 5 });
    expect(s.getState().entities).toEqual([]);
  });

  it('placeEntity on hole is rejected', () => {
    const s = makeStore();
    s.getState().setCellKind({ x: 8, y: 5 }, 'hole');
    s.getState().placeEntity('ally', { x: 8, y: 5 });
    expect(s.getState().entities).toEqual([]);
  });

  it('unique kinds (me, harebourg) replace existing', () => {
    const s = makeStore();
    s.getState().placeEntity('me', { x: 8, y: 5 });
    s.getState().placeEntity('me', { x: 9, y: 5 });
    const me = s.getState().entities.filter((e) => e.kind === 'me');
    expect(me).toHaveLength(1);
    expect(me[0].cell).toEqual({ x: 9, y: 5 });
  });
